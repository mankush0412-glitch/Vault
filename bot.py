...
async def main_menu_buttons(user_id: int) -> list:
    rows = [
        [color_btn("🛒 ʙᴜʏ ᴀᴄᴄᴏᴜɴᴛ", "store", "success"),
    ...
    if await get_setting("whatsapp_enabled", False):
        rows.append([color_btn("💬 ᴡʜᴀᴛsᴀᴘᴘ", "whatsapp", "primary")])
...
def _inventory_buttons(countries: List[dict], page: int = 0, per_page: int = 8) -> list:
    ...
    for i in range(start, end, 2):
        ...
        for c in countries[i:i+2]:
            stock = c["stock"]
            price = c["price"]
            info_text = f"{c['flag']} {c['name']}  {stock}  {price} ᴄʀ"
...
async def callback_router(event):
    ...
    # ── BUY ──────────────────────────────────────────────────────
        ...
        text = fancy(
            ...
            f"💰 ᴘʀɪᴄᴇ: `{price} ᴄʀ`\n"
            f"💎 ʏᴏᴜʀ ʙᴀʟᴀɴᴄᴇ: `{bal:.2f} ᴄʀ`"
        )
        await event.edit(text, buttons=[
            [color_btn("✅ ᴄᴏɴꜰɪʀᴍ", f"confirm_buy:{code}", "success"),
    ...
    # ─── DEPOSIT – PACKAGES ──────────────────────────────────────
        ...
        rows.append([color_btn("💡 ᴄᴜsᴛᴏᴍ ᴀᴍᴏᴜɴᴛ", "custom_deposit", "default")])
        rows.append([color_btn("🏠 ʜᴏᴍᴇ", "main_menu", "default")])
        await event.edit(
        ...
        methods = [
            [color_btn("💳 ᴜᴘɪ / Qʀ – ₹{}".format(pkg['price']), f"pay_method:upi:{credits}", "success")],
            [color_btn("₿ ᴜsᴅᴛ (ᴄʀʏᴘᴛᴏ) – {} USDT".format(pkg['usdt']), f"pay_method:usdt:{credits}", "primary")],
            [color_btn("⭐ ᴛᴇʟᴇɢʀᴀᴍ sᴛᴀʀs – {} ⭐".format(pkg['stars']), f"pay_method:stars:{credits}", "primary")],
        ...
        if method == "upi":
            ...
            await event.edit(msg, buttons=[
                [color_btn("📋 ᴄᴏᴘʏ ᴀᴅᴅʀᴇss", f"copy_addr:{crypto_addr}", "primary")],
                [color_btn("✅ ɪ ʜᴀᴠᴇ ᴘᴀɪᴅ", "deposit_paid", "success")],
                [color_btn("🔄 ᴄʜᴇᴄᴋ ᴘᴀʏᴍᴇɴᴛ", "check_payment", "primary")],
                [color_btn("◀️ ʙᴀᴄᴋ", "deposit", "default")]
            ])
    ...
    # ── UPLOAD SESSIONS ─────────────────────────────────────────
        ...
        for c in c_list:
            rows.append([color_btn(
                f"{c['flag']} {c['name']}",
                f"upload_country:{c['code']}:{cat_name}",
                "default"
            )])
        rows.append([color_btn("◀️ ʙᴀᴄᴋ", "admin", "default")])
        ...
        for dep in deps:
            ...
            await bot.send_message(
                ...
                buttons=[
                    [color_btn(
                        "✅ ᴀᴘᴘʀᴏᴠᴇ",
                        f"dep_approve:{dep_id}:{uid}:{amount}",
                        "success"
                    ),
                    color_btn(
                        "❌ ʀᴇᴊᴇᴄᴛ",
                        f"dep_reject:{dep_id}:{uid}",
                        "danger"
                    )],
                ],
            )
    ...
    # ── SETTINGS ──────────────────────────────────────────────────
        ...
        await event.edit(
            ...
            buttons=[
                [color_btn("🤖 ʙᴏᴛ ɴᴀᴍᴇ", "set_botname", "primary"),
                 color_btn("💳 ᴜᴘɪ ɪᴅ", "set_upi", "primary")],
                [color_btn("👤 ᴜᴘɪ ɴᴀᴍᴇ", "set_upiname", "primary"),
                 color_btn("📞 sᴜᴘᴘᴏʀᴛ", "set_support", "primary")],
                [color_btn("🎁 ʀᴇꜰ ʙᴏɴᴜs", "set_ref_bonus", "primary"),
                 color_btn("📈 ʀᴇꜰ %", "set_ref_pct", "primary")],
                [color_btn("🔢 ᴍɪɴ ᴅᴇᴘᴏsɪᴛ", "set_min_dep", "primary")],
                [color_btn("💬 ᴛᴏɢɢʟᴇ ᴡʜᴀᴛsᴀᴘᴘ", "toggle_whatsapp", "primary"),
                 color_btn("🖼️ sᴇᴛ ᴡᴇʟᴄᴏᴍᴇ ᴘʜᴏᴛᴏ", "set_welcome_photo", "primary")],
                [color_btn("⏳ ᴀᴜᴛᴏ ᴠᴇʀɪғʏ ᴅᴇʟᴀʏ", "set_verify_delay", "primary"),
                 color_btn("🌐 ᴘᴀʏᴍᴇɴᴛ ᴡᴇʙʜᴏᴏᴋ", "set_webhook", "primary")],
    ...
    # ── ADD COUNTRY ──────────────────────────────────────────────
        ...
        try:
            ...
            await event.respond(fancy(msg),
                                buttons=[[color_btn("◀️ ᴄᴏᴜɴᴛʀɪᴇs", "acountries", "default")]])
        except Exception:
            await event.respond(
