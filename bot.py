...
def button_label(text: str) -> str:
    """Use full-size lettering on buttons while keeping fancy message text."""
...
async def main_menu_buttons(user_id: int) -> list:
    ...
    # Keep the two secondary actions together; primary actions stay full-width.
    rows.append([
...
def _category_buttons(categories: List[dict]) -> list:
    rows = []
    # One category per row gives every category a comfortable full-width button.
    for cat in categories:
        icon = cat.get("icon", "📦")
        name = str(cat.get("name", "Category")).strip()
        rows.append([
            color_btn(f"{icon} {name}", f"category:{name}", "primary")
        ])
...
def _inventory_buttons(countries: List[dict], page: int = 0, per_page: int = 8) -> list:
    ...
    rows  = []
    # Full-width purchase rows are easier to read on every phone size.
    for c in countries[start:end]:
        stock = c["stock"]
        price = c["price"]
        # Step 3: Credit system (cr) instead of ₹
        info_text = f"{c['flag']} {c['name']}  •  {stock} IN STOCK  •  {price} ᴄʀ"
        rows.append([color_btn(info_text, f"buy:{c['code']}", "success")])
...
async def cmd_start(event):
    ...
    raw_welcome = (
        ...
        "✨ **ᴡʜᴀᴛ ʏᴏᴜ ᴄᴀɴ ᴅᴏ:**\n"
        "• ʙᴜʏ ᴛᴇʟᴇɢʀᴀᴍ ᴀᴄᴄᴏᴜɴᴛs ᴡɪᴛʜ ɪɴsᴛᴀɴᴛ OTP & 2FA ᴅᴇᴛᴀɪʟs\n"
        "• ᴀᴅᴅ ᴄʀᴇᴅɪᴛs ᴠɪᴀ UPI, USDT ᴏʀ Tᴇʟᴇɢʀᴀᴍ Sᴛᴀʀs\n"
        "• ᴄʜᴏᴏsᴇ ʏᴏᴜʀ ᴄᴀᴛᴇɢᴏʀʏ, ᴄᴏᴜɴᴛʀʏ ᴀɴᴅ ᴘʀɪᴄᴇ\n"
        "• ᴄʜᴇᴄᴋ sᴛᴏᴄᴋ ᴀɴᴅ ᴘʀɪᴄᴇ ʙᴇғᴏʀᴇ ʏᴏᴜ ʙᴜʏ\n\n"
...
async def callback_router(event):
    ...
    if data == "main_menu":
        ...
        await event.edit(
            fancy(
                f"🏠 **{bot_name}**\n\n"
                "✨ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴏᴜɴᴛs • ғᴀsᴛ ᴅᴇʟɪᴠᴇʀʏ\n\n"
                "• ʙʀᴏᴡsᴇ ᴀᴄᴄᴏᴜɴᴛs ʙʏ ᴄᴀᴛᴇɢᴏʀʏ & ᴄᴏᴜɴᴛʀʏ\n"
                "• ᴘᴀʏ ᴡɪᴛʜ ᴄʀᴇᴅɪᴛs ᴀɴᴅ ɢᴇᴛ ᴅᴇᴛᴀɪʟs ɪɴsᴛᴀɴᴛʟʏ\n\n"
                "👇 ᴄʜᴏᴏsᴇ ᴀɴ ᴏᴘᴛɪᴏɴ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ:"
            ),
            buttons=await main_menu_buttons(user_id),
        )
    ...
    # ── STORE (categories) ──────────────────────────────────────
        ...
        await event.edit(
            fancy(
                "📂 **sᴇʟᴇᴄᴛ ᴀᴄᴄᴏᴜɴᴛ ᴄᴀᴛᴇɢᴏʀʏ**\n\n"
                "✨ ᴄʜᴏᴏsᴇ ᴛʜᴇ ᴄᴀᴛᴇɢᴏʀʏ ᴛʜᴀᴛ ʙᴇsᴛ ғɪᴛs ʏᴏᴜʀ ɴᴇᴇᴅ.\n"
                "• sᴛᴏᴄᴋ & ᴘʀɪᴄᴇ ᴀʀᴇ sʜᴏᴡɴ ʙᴇғᴏʀᴇ ᴘᴜʀᴄʜᴀsᴇ\n"
                "• ᴇᴀᴄʜ ᴄᴀᴛᴇɢᴏʀʏ ᴏᴘᴇɴs ɪᴛs ᴀᴠᴀɪʟᴀʙʟᴇ ᴄᴏᴜɴᴛʀɪᴇs\n\n"
        ...
        if not countries:
            await event.edit(
                fancy(
                    f"😔 **ɴᴏ ᴀᴄᴄᴏᴜɴᴛs ᴀᴠᴀɪʟᴀʙʟᴇ ɪɴ '{cat_name}'.**\n\n"
                    "• ᴛʜɪs ᴄᴀᴛᴇɢᴏʀʏ ɪs ᴄᴜʀʀᴇɴᴛʟʏ ᴏᴜᴛ ᴏғ sᴛᴏᴄᴋ\n"
                    "• ᴘʟᴇᴀsᴇ ᴄʜᴏᴏsᴇ ᴀɴᴏᴛʜᴇʀ ᴄᴀᴛᴇɢᴏʀʏ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ"
                ),
                buttons=[[color_btn("◀️ ʙᴀᴄᴋ ᴛᴏ ᴄᴀᴛᴇɢᴏʀɪᴇs", "store", "primary")]]
            )
            return
        await event.edit(
            fancy(
                f"📌 **{cat_name}**\n\n"
                "🌍 ᴄʜᴏᴏsᴇ ᴀ ᴄᴏᴜɴᴛʀʏ ᴛᴏ ᴠɪᴇᴡ ᴀᴠᴀɪʟᴀʙʟᴇ sᴛᴏᴄᴋ ᴀɴᴅ ᴘʀɪᴄᴇ.\n"
                "• ᴇᴀᴄʜ ᴏᴘᴛɪᴏɴ sʜᴏᴡs sᴛᴏᴄᴋ, ᴘʀɪᴄᴇ & ᴄʀᴇᴅɪᴛ ᴠᴀʟᴜᴇ\n"
                "👇 ᴛᴀᴘ ᴀɴ ᴏᴘᴛɪᴏɴ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ:"
            ),
        ...
        await event.edit(
            fancy(
                f"📌 **{cat_name or 'ᴀʟʟ'}**\n\n"
                "🌍 ᴄʜᴏᴏsᴇ ᴀ ᴄᴏᴜɴᴛʀʏ ᴛᴏ ᴠɪᴇᴡ ᴀᴠᴀɪʟᴀʙʟᴇ sᴛᴏᴄᴋ ᴀɴᴅ ᴘʀɪᴄᴇ.\n"
                "• ᴛᴀᴘ ᴀɴ ᴏᴘᴛɪᴏɴ ʙᴇʟᴏᴡ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ:"
            ),
    ...
    # ─── DEPOSIT – PACKAGES ──────────────────────────────────────
        ...
        rows = []
    # Two packages per row keeps the package selector compact and balanced.
        for i in range(0, len(pkgs), 2):
            row = []
            for p in pkgs[i:i+2]:
                row.append(color_btn(f"💎 {p['credits']} ᴄʀᴇᴅɪᴛs · ₹{p['price']}", f"pkg:{p['credits']}", "success"))
            rows.append(row)
        rows.append([color_btn("✏️ ᴄᴜsᴛᴏᴍ ᴀᴍᴏᴜɴᴛ", "custom_deposit", "primary")])
        rows.append([color_btn("🏠 ʜᴏᴍᴇ", "main_menu", "default")])
        await event.edit(
            fancy(
                "💳 **ᴀᴅᴅ ᴄʀᴇᴅɪᴛs & ᴛᴏᴘ-ᴜᴘ**\n\n"
                f"• ᴘᴜʀᴄʜᴀsᴇ ᴄʀᴇᴅɪᴛs: `{current_balance:.2f} ᴄʀ`\n"
                f"• ᴡɪᴛʜᴅʀᴀᴡᴀʙʟᴇ ʙᴀʟᴀɴᴄᴇ: `{withdrawable_balance:.2f} ᴄʀ`\n"
                f"• ᴛᴏᴛᴀʟ ᴘᴜʀᴄʜᴀsɪɴɢ ᴘᴏᴡᴇʀ: `{current_balance + withdrawable_balance:.2f} ᴄʀ`\n\n"
                "⚡ **ɪɴsᴛᴀɴᴛ & ᴀᴜᴛᴏᴍᴀᴛᴇᴅ**\n"
                "💡 ʀᴀᴛᴇ: `1 ᴄʀ = ₹1 / $0.01`\n\n"
                "👇 sᴇʟᴇᴄᴛ ᴀ ᴄʀᴇᴅɪᴛ ᴘᴀᴄᴋᴀɢᴇ ʙᴇʟᴏᴡ:"
            ),
            buttons=rows
        )
        ...
        await event.edit(
            fancy(
                f"💳 **sᴇʟᴇᴄᴛ ᴘᴀʏᴍᴇɴᴛ ᴍᴇᴛʜᴏᴅ**\n\n"
                f"📦 ᴘᴀᴄᴋᴀɢᴇ: `{credits} ᴄʀᴇᴅɪᴛs`\n"
                f"💰 ᴠᴀʟᴜᴇ: ₹{pkg['price']} | {pkg['usdt']} USDT | {pkg['stars']} ⭐\n\n"
                "✨ ᴄʜᴏᴏsᴇ ʏᴏᴜʀ ᴘʀᴇғᴇʀʀᴇᴅ ɢᴀᴛᴇᴡᴀʏ ʙᴇʟᴏᴡ.\n"
                "⚡ ᴘᴀʏᴍᴇɴᴛs ᴀʀᴇ ᴠᴇʀɪғɪᴇᴅ ᴀɴᴅ ᴄʀᴇᴅɪᴛᴇᴅ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ.\n"
                "💡 ʀᴀᴛᴇ: `1 ᴄʀ = ₹1 / $0.01`"
            ),
            buttons=methods
        )
